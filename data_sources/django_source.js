  ...
  fetch: function(store, query) {
   ...
   /* console.log('queryCallback'+ query.callbackFunction);
    ...
    if(query.orderBy || query.parameters || query.conditions || query.relations) {
        ...
        for(var i in query.parameters) {
          ...
          for (var j = 0; j<objects.length; j++)
          {
            if (i == objects[j])
            {
              ...
              if (objects[j] != 'limit')
              {
                parametersLength=parametersLength+""+objects[j]+"="+query.parameters[i].get('pk')+"&";   
              }
          ...
          if (!doNothing)
          {
            ...
            if (i!='limit')
            {
              parametersLength=parametersLength+""+i+"="+query.parameters[i]+"&";   
            }
      ...
      lengthUrl = Django.getLengthURL.fmt(Django.urlPrefix, appName, modelName) ;
    }

    var reqObject = SC.Request.extend({
      store: store,
      recordType: recordType,
      didReceive: function(par1, par2) {
        sc_super();
        var key = this.get('recordType').toString().replace('.','_')+'_length';
        this.get('store').set(key, parseInt(par2.body(),10));
      },
    ...
    if (lengthUrl)
    {
      var request1 = reqObject.getUrl(lengthUrl).send();  
    }
    query.url = url;
      SC.Request.getUrl(url).set('isJSON', YES)
        .notify(200, this, this._didFetchRecords, { 
           query: query, store: store
         }).send();


      
      //SC.Request.getUrl(lengthUrl).send();
      return YES ;
    } else return NO ;
  },


   _didFetchLength2: function(request) {
    console.log("_didFetchLength2");
